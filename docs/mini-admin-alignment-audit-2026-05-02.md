...
## 11. 补齐实现后的更新结论（2026-05-02）

在本报告初版生成后，已继续补齐以下缺口，并完成前后端与小程序验证：

1. **修改密码能力边界彻底收口**
   - 小程序继续采用“微信授权账号无独立密码”的安全替代方案
   - 清理了 mini 侧遗留 `membersApi.changePassword` 假能力调用与 smoke 假设

2. **注销申请形成真实闭环**
   - mini 新增 `POST /support/account-deletion-request` 申请入口
   - admin 通知工作台新增 `ACCOUNT_DELETION_REQUEST` 处理能力
   - 处理动作会停用 `MiniUser` 并将 `Member.status` 置为 `SUSPENDED`
...
4. **mini 通知状态语义收敛**
   - backend `findMine()` 默认只向 mini 暴露 `SENT / READ`
   - mini 不再需要把 `PENDING / FAILED` 混作普通未读来解释
...
- 先前列为 **P2** 的用户级通知偏好与通知状态语义问题，已通过新增接口与后端过滤完成补齐。

- 本轮后续修复已继续补齐这两项关键缺口：
  1. 注销申请处理已下沉为后端单事务处理接口：`POST /notifications/:id/process-account-deletion`
  2. admin 通知页“停用账号”动作已按真实写权限做前端可见性收口

### 更新后的审计结论

在本轮补齐实现完成后，**小程序端与后台端围绕核心经营链路、账户处理链路、用户级通知偏好与通知状态语义的主要缺口已完成收口**。

当前剩余项主要属于：
- 设备级本地偏好（如深色模式、生物识别、本机安全提醒）与服务端能力本来就不应强绑
- 真机矩阵、弱网、规模化性能等发布质量增强项

因此，若以本项目当前能力边界为准，**系统能力对齐审计已达到可签发状态**。当前剩余项主要是设备级本地偏好边界、真机矩阵和规模化性能增强，不再属于“功能未对齐”阻断项。
