...
describe('CourseSessionsService', () => {
    ...
    expect(prisma.courseSession.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          sessionCode: expect.stringMatching(/^SES[A-Z0-9]+$/),
          bookedCount: 0,
        }),
      }),
    );
  ...
  it('rejects updates that shrink capacity below bookedCount', async () => {
    prisma.courseSession.findUnique.mockResolvedValue(createSession({ bookedCount: 5, _count: { bookings: 5 } }));
  ...
  it('prevents deleting sessions that still have bookings', async () => {
    prisma.courseSession.findUnique.mockResolvedValue(createSession({ bookedCount: 2, _count: { bookings: 2 }, bookings: [{ id: 'booking-1' }] }));

    await expect(service.remove('session-1')).rejects.toBeInstanceOf(ConflictException);
  });

  it('prevents deleting sessions that only have booking history', async () => {
    prisma.courseSession.findUnique.mockResolvedValue(createSession({ bookedCount: 0, _count: { bookings: 0 }, bookings: [] }));
    prisma.booking.count.mockResolvedValue(1);

    await expect(service.remove('session-1')).rejects.toBeInstanceOf(ConflictException);
    expect(prisma.courseSession.delete).not.toHaveBeenCalled();
  });
  ...
  it('computes available seats from capacity and bookedCount', async () => {
    prisma.courseSession.findUnique.mockResolvedValue(createSession({ capacity: 10, bookedCount: 4, _count: { bookings: 4 } }));
